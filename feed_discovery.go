...
func discoverFeeds(pageURL string) (FeedDiscoveryResult, error) {
	...
	if foundPattern(baseURL, commonPatterns) {
	...
	for _, pattern := range commonPatterns {
		...
		log.Println(isValidFeed(testURL), isNewFeedUrl(result.FeedURLs, testURL))
...
func foundPattern(url string, commonPatterns []string) bool {

	for _, eachPattern := range commonPatterns {
		if strings.Contains(url, eachPattern) {
			return true
		}
	}
...
func isValidFeed(url string) bool {
	...
	// log.Println(strings.Contains(content, "<rss"))
